...
async def process_teacher_search_logic(text: str, message: Message, state: FSMContext):
    ...
    if not results:
        ...
        keyboard = get_teacher_card_keyboard(teacher['id'], monday.isoformat())
        await state.clear()
    ...
    if not text.isdigit():
        ...
        if last_msg_id:
            try:
                ...
                pass
        await message.answer(err_text, reply_markup=get_teacher_search_cancel_keyboard())
        return
...
async def cb_teacher_today(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    keyboard = get_teacher_today_keyboard(teacher_id, monday.isoformat())
    await callback.message.edit_text(text, reply_markup=keyboard)
    await callback.answer()
...
async def cb_teacher_tomorrow(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    keyboard = get_teacher_tomorrow_keyboard(teacher_id, monday.isoformat())
    await callback.message.edit_text(text, reply_markup=keyboard)
    await callback.answer()
...
async def cb_teacher_week(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    for lesson in lessons_list:
        week_schedule.setdefault(lesson['date'], []).append(lesson)
    ...
    if day_index is None:
        ...
        keyboard = get_teacher_week_navigation_keyboard(teacher_id, monday, None)
    else:
        # Detailed day view
        ...
        keyboard = get_teacher_week_navigation_keyboard(teacher_id, monday, day_index)
